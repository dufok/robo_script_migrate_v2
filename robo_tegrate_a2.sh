...
# Look, this is not for crontab, this is migration for single use. Test purpose only.
#
# dufok was here (^.^')
# 
# This script is used to migrate the Discoursio API from the old server to the new one.
# It will:
# The structure is as follows:
# 1. Increase the swap space to 6GB
# 2. Check if the container is running
# 3. Check if the dump and data directories exist
# 4. Check if the old database is set
# 5. Check if the dump and data directories exist in the application configuration
# 6. Untar the bson file from the mongoDB dump
# 7. Set the application to bson mode
# 8. Create a new postgres database
# 9. Migrate the json files
# 10. Restart the application and clean up
#
# Main  starter script is in the End of the file
